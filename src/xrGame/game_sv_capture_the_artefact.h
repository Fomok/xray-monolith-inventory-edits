...
private:
	...
	struct MinPlayersFunctor : public std::function<bool(TeamPair, TeamPair)>
	{
		bool operator()(const TeamPair& left, const TeamPair& right) const;
	};

	struct SearchArtefactIdFunctor : public std::function<bool(TeamPair, u16)>
	{
		bool operator()(const TeamPair& tr, u16 artefactId) const;
	};

	struct SearchOwnerIdFunctor : public std::function<bool(TeamPair, u16)>
	{
