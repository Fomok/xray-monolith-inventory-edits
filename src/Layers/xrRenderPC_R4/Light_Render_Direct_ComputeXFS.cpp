...
void CLight_Compute_XFORM_and_VIS::compute_xf_spot(light* L)
{
	...
	// final size calc
	u32 max_size = RImplementation.o.smapsize <= ps_ssfx_shadows.y ? RImplementation.o.smapsize : (u32)ps_ssfx_shadows.y;
	u32 _size = iFloor(factor * SMAP_adapt_optimal);
	if (_size < ps_ssfx_shadows.x) _size = (u32)ps_ssfx_shadows.x;
